...
export const trabajoSchema = yup.object().shape({
    ...
    kilometraje: yup.number().integer().positive('El kilometraje debe ser un número positivo'),
    patente: yup.string().matches(/^[A-Z0-9]{7}$/, 'La patente debe tener 7 caracteres alfanuméricos'),
    id_proyecto: yup.number().required('El id de proyecto es requerido').positive('El id de proyecto debe ser un número positivo'),
    dni_conductor: yup.string().matches(/^[0-9]{8}$/, 'El dni del conductor debe tener 8 dígitos'),
    cuit_cliente: yup.string().matches(/^[0-9]{11}$/, 'El cuit del cliente debe tener 11 dígitos'),
});